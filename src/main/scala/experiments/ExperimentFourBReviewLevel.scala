...
class ExperimentFourBReviewLevel(
    targetFeedbackType: FeedbackTypes.Value,
    sc: SparkContext,
    db: DataSource
  ) extends Experiment {

  override val name = "SVM Mixed Labels B"
  ...
  override def run = {
    ...
    val trainingLabelsNoisy: Seq[Int] = noisyLabelProvider.getReviewLabels(trainingDocsNoisy)
